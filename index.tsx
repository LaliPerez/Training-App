...
import { 
  FileText, CheckCircle, ArrowLeft, Send, 
  PlusCircle, Users, FileDown, LogOut, Trash2, 
  Edit, X, Share2, Copy, Eye, 
...
const UserPortal: React.FC<UserPortalProps> = ({ trainings, setTrainings, addUserSubmission, onBack }) => {
  ...
  const [formData, setFormData] = useState({
    firstName: '', lastName: '', dni: '', company: '', email: '', phone: '', signature: '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submission: Omit<UserSubmission, 'id'> = {
        trainingId: selectedTraining.id, trainingName: selectedTraining.name,
  ...
  if (formCompleted) {
    return (
      <div className="text-center p-8 bg-white rounded-lg shadow-xl">
        ...
        <button
          onClick={() => { setSelectedTrainingId(null); setFormCompleted(false); }}
      ...
      <div className="w-full max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg">
        ...
        <div className="space-y-3 mb-8">
          {selectedTraining.links.map(link => (
            <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" onClick={() => handleLinkClick(selectedTraining.id, link.id)} className={`flex items-center justify-between p-4 rounded-lg border transition-all ${link.viewed ? 'bg-green-50 border-green-300 text-gray-500' : 'bg-white border-gray-300 hover:bg-gray-50'}`}>
        ...
        {allLinksViewed && (
          <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
            ...
            <button type="submit" disabled={!formData.signature} className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-gray-400 disabled:cursor-not-allowed">
              <Send className="h-5 w-5 mr-2" /> Enviar Información
            </button>
          </form>
        )}
      </div>
      ...
      <div className="space-y-4">
        {trainings.length > 0 ? (
          trainings.map(training => (
            <button key={training.id} onClick={() => setSelectedTrainingId(training.id)} className="w-full text-left p-6 bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow border border-gray-200">
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            ...
            <input id="password" name="password" type="password" autoComplete="current-password" required value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Contraseña"/>
          </div>
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ trainings, userSubmissions, addTraining, updateTraining, deleteTraining, onLogout }) => {
  ...
  const handleAddTraining = (e: React.FormEvent) => {
    e.preventDefault();
    if (!trainingName.trim() || !linksText.trim()) { setFeedback('El nombre y los enlaces no pueden estar vacíos.'); return; }
    const links = linksText.split('\n').filter(link => link.trim() !== '');
    if (links.length === 0) { setFeedback('Debe proporcionar al menos un enlace válido.'); return; }
    addTraining(trainingName, links);
    setTrainingName(''); setLinksText('');
  ...
  const handleUpdateTraining = (e: React.FormEvent) => {
    ...
    if (!editedName.trim() || links.length === 0) { alert("El nombre y los enlaces no pueden estar vacíos."); return; }
    updateTraining(editingTraining.id, editedName, links);
    setEditingTraining(null);
  };
  const handleDeleteTraining = (id: string) => {
    if(window.confirm('¿Estás seguro de que quieres eliminar esta capacitación? Esta acción no se puede deshacer.')) { deleteTraining(id); }
  };
  const handleShare = () => {
  ...
  const handleSaveAdminSignature = () => {
    if (adminSignatureRef.current) {
      if (adminSignatureRef.current.isEmpty()) { alert("Por favor, dibuja tu firma antes de guardar."); return; }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="space-y-8">
          <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            <form onSubmit={handleAddTraining} className="space-y-4">
              <input id="trainingName" type="text" value={trainingName} onChange={(e) => setTrainingName(e.target.value)} placeholder="Ej: Inducción de Seguridad" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"/>
              <textarea id="links" value={linksText} onChange={(e) => setLinksText(e.target.value)} rows={4} placeholder="https://ejemplo.com/link1&#10;https://ejemplo.com/link2" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"/>
              <div className="flex items-center justify-between">
                <button type="submit" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  <PlusCircle className="h-5 w-5 mr-2" /> Agregar Capacitación
                </button>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center gap-2">
                <button onClick={openUserView} className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700">
                  <Eye className="h-4 w-4 mr-2" /> Vista de Usuario
                </button>
                <button onClick={handleShare} disabled={trainings.length === 0} className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-teal-600 hover:bg-teal-700 disabled:bg-gray-400">
              ...
              {trainings.length > 0 ? (
                trainings.map(training => (
                  <div key={training.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => setEditingTraining(training)} className="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded-full transition-colors"><Edit className="h-4 w-4" /></button>
                      <button onClick={() => handleDeleteTraining(training.id)} className="p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded-full transition-colors"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
          <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
            ...
            <div className="flex gap-2 flex-wrap items-center">
              <button onClick={() => setShowAdminSignatureModal(true)} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50">
                <Edit className="h-4 w-4 mr-2" /> {adminSignature ? 'Cambiar Firma' : 'Configurar Firma'}
              </button>
              {adminSignature && <img src={adminSignature} alt="Admin signature preview" className="h-10 w-20 object-contain border rounded-md bg-gray-50 p-1" />}
              <button onClick={() => generateSubmissionsPdf(userSubmissions, adminSignature!)} disabled={userSubmissions.length === 0 || !adminSignature} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed">
                <FileDown className="h-5 w-5 mr-2" /> Descargar Constancia
              </button>
            </div>
          </div>
          <div className="overflow-x-auto">
            {userSubmissions.length > 0 ? (
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50"><tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nombre</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DNI</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Capacitación</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha</th></tr></thead>
      ...
      {editingTraining && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">...</div>)}
      {showShareModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">...</div>)}
      {selectedSubmission && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">...</div>)}
      {showAdminSignatureModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">...</div>)}
    </div>
  );
};
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (data) {
        ...
        if (Array.isArray(decoded)) {
          setTrainings(decoded);
          localStorage.setItem('trainings', JSON.stringify(decoded));
          setView('user');
          urlWasModified = true;
        }
      } else {
  ...
  const addTraining = (name: string, urls: string[]) => {
    const newTraining: Training = {
      id: `training-${Date.now()}`, name,
      links: urls.map((url, i) => ({ id: `link-${Date.now()}-${i}`, url, viewed: false })),
    };
    setTrainings(prev => [...prev, newTraining]);
  };
  const updateTraining = (id: string, name: string, urls: string[]) => {
    setTrainings(prev => prev.map(t => t.id === id ? { ...t, name, links: urls.map((url, i) => t.links.find(l => l.url === url) || { id: `link-${Date.now()}-${i}`, url, viewed: false }) } : t));
  };
  const deleteTraining = (id: string) => {
    setTrainings(prev => prev.filter(t => t.id !== id));
  };
  const addUserSubmission = (submission: Omit<UserSubmission, 'id'>) => {
    const newSubmission: UserSubmission = { ...submission, id: `sub-${Date.now()}` };
    setUserSubmissions(prev => [...prev, newSubmission]);
  };
  
  const renderView = () => {
    switch (view) {
      case 'login': return <AdminLogin onLoginSuccess={() => setView('admin')} onBack={() => setView('selector')} />;
      case 'admin': return <AdminDashboard trainings={trainings} userSubmissions={userSubmissions} addTraining={addTraining} updateTraining={updateTraining} deleteTraining={deleteTraining} onLogout={() => setView('selector')} />;
      case 'user': return <UserPortal trainings={trainings} setTrainings={setTrainings} addUserSubmission={addUserSubmission} onBack={() => setView('selector')} />;
      case 'selector':
      default:
        return (
          <div className="text-center">
            ...
            <div className="mt-8 flex justify-center gap-4 md:gap-8">
              <button onClick={() => setView('login')} className="flex flex-col items-center justify-center w-48 h-48 p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border">
              ...
              <button onClick={() => setView('user')} className="flex flex-col items-center justify-center w-48 h-48 p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border">
