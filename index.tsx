...
import { 
  ...
  WifiOff,
  AlertCircle,
  // Added Key icon import to fix line 678 error
  Key
} from 'lucide-react';
...
// --- Config & API ---
const STORAGE_KEYS = { 
  MASTER_KEY: 'trainer_master_key_v25',
  WSID: 'trainer_ws_id_v25', 
  AUTH: 'trainer_auth_v25',
  STATE_CACHE: 'trainer_state_cache_v25'
};
const API_URL = 'https://api.restful-api.dev/objects';

const api = {
  // Función robusta para cargar datos evitando sobrescribir con nulos
  load: async (id: string): Promise<AppState | null> => {
    try {
      ...
      if (data && Array.isArray(data.clients) && Array.isArray(data.modules)) {
        return data;
      }
      return null;
  ...
  save: async (id: string, masterKey: string, data: AppState): Promise<boolean> => {
    try {
      const res = await fetch(`${API_URL}/${id}`, {
        ...
        body: JSON.stringify({ name: `TrainerCloud_${masterKey}`, data })
      });
      return res.ok;
...
const App = () => {
  ...
  // Sincronización proactiva (cada 8 seg)
  ...
  const updateGlobal = async (patch: Partial<AppState>) => {
    ...
    // FETCH-AND-MERGE: Obtenemos lo último para no borrar lo que otros hayan subido
    ...
    const newState = { 
      ...
      instructor: patch.instructor || baseState.instructor
    };
        ...
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-800 ${syncStatus === 'error' ? 'bg-red-500/10' : 'bg-slate-900'}`}>
            ...
            <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">
              {syncStatus === 'syncing' ? 'Cloud Syncing' : syncStatus === 'error' ? 'Sync Error' : 'Cloud Online'}
            </span>
          </div>
      ...
      <main className="pt-24 px-4 max-w-7xl mx-auto pb-20">
        {view === 'landing' && (
          <div className="min-h-[70vh] flex flex-col items-center justify-center text-center animate-in fade-in">
            ...
            <p className="text-slate-500 font-bold uppercase tracking-[0.3em] text-[clamp(0.6rem,2vw,0.8rem)] mb-12 italic">Solución de Capacitación y Firma Digital</p>
          ...
          <div className="animate-in slide-in-from-bottom-6">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 gap-8">
              <div className="space-y-3">
                ...
                <div className="flex bg-[#111827] px-4 py-2 rounded-full border border-gray-800 items-center gap-2">
                   <KeyIcon size={12} className="text-blue-500" />
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Workspace: {masterKey}</span>
                </div>
              </div>
              <div className="flex bg-[#111827] p-1.5 rounded-2xl border border-gray-800 overflow-x-auto shadow-xl no-scrollbar">
                {['asistencias', 'asignaciones', 'modulos', 'clientes', 'instructor'].map(t => (
                  <button key={t} onClick={() => setAdminTab(t as any)} className={`flex items-center gap-2 px-6 py-4 rounded-xl font-black uppercase tracking-widest text-[9px] transition-all whitespace-nowrap ${adminTab === t ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>
                    {t}
                  </button>
                ))}
              </div>
        ...
        {view === 'userForm' && <UserPortal state={state} wsid={wsid} masterKey={masterKey} onSync={() => sync(false)} onSubmit={async (rec) => {
          if (!wsid) return;
          // FLUJO CRÍTICO DE GUARDADO: Fetch -> Merge -> Save
          const cloud = await api.load(wsid);
          if (!cloud) {
             throw new Error("No se pudo obtener el estado de la nube");
          }
          const updated = { ...cloud, records: [rec, ...(cloud.records || [])] };
          const ok = await api.save(wsid, masterKey, updated);
          if (!ok) throw new Error("Error de guardado");
          setState(updated);
        }} />}
      </main>
    </div>
      ...
      <div className="bg-[#111827] p-8 md:p-12 rounded-[3.5rem] border border-gray-800 w-full max-w-md text-center shadow-2xl">
        ...
        <div className="space-y-6 mb-10 text-left">
          <div className="relative group">
            ...
            <input type="password" placeholder="Clave Maestra..." value={val} onChange={(e) => setVal(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && onLogin(val)} className="w-full bg-[#0d111c] border border-gray-800 text-white pl-16 pr-6 py-6 rounded-2xl outline-none font-bold text-lg focus:border-blue-500" />
          </div>
        </div>
    ...
    autoTable(doc, {
      ...
      didDrawCell: (d: any) => {
        if (d.section === 'body' && d.column.index === 4) {
          ...
          if (rec?.signature) doc.addImage(rec.signature, 'PNG', d.cell.x + 2, d.cell.y + 2, 36, 21);
        }
      },
    ...
    <div className="animate-in fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button onClick={() => confirm("¿Borrar?") && update({ records: state.records.filter((r: any) => !sel.includes(r.id)) })} disabled={!sel.length} className="flex-1 bg-red-500/10 text-red-500 px-6 py-3 rounded-xl font-black text-[10px] disabled:opacity-20">Borrar</button>
          <button onClick={generatePDF} disabled={!sel.length} className="flex-1 bg-blue-600 text-white px-8 py-3 rounded-xl font-black text-[10px] shadow-lg disabled:opacity-30">PDF</button>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-800/50">
            {(state.records || []).map((r: any) => (
              <tr key={r.id} className="hover:bg-blue-600/5 cursor-pointer" onClick={() => setSel(s => s.includes(r.id) ? s.filter(i => i !== r.id) : [...s, r.id])}>
                ...
                <td className="px-6 py-6 text-center">{r.signature && <img src={r.signature} className="h-10 mx-auto bg-white rounded p-1 shadow-sm border border-gray-800" />}</td>
...
const AsignacionesView = ({ state, update, wsid, masterKey }: any) => {
  ...
  return (
    <div className="animate-in fade-in text-left">
      <h2 className="text-white text-2xl font-black italic uppercase mb-10">Vínculos de Acceso</h2>
      <div className="bg-[#0d111c] p-6 rounded-[2.5rem] border border-gray-800 flex flex-col lg:flex-row gap-6 items-end mb-12 shadow-inner">
        <div className="flex-1 w-full space-y-2">
          <label className="text-slate-600 text-[10px] font-black uppercase px-2">Cliente</label>
        ...
        <div className="flex-1 w-full space-y-2">
          <label className="text-slate-600 text-[10px] font-black uppercase px-2">Curso</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {(state.assignments || []).map((a: any) => (
          <div key={a.id} className="bg-[#0d111c] p-8 rounded-[3rem] border border-gray-800 flex flex-col shadow-lg">
            ...
            }} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-[10px] hover:bg-blue-700 shadow-lg">Ver Código QR</button>
            <button onClick={() => confirm("¿Borrar?") && update({ assignments: state.assignments.filter((as: any) => as.id !== a.id) })} className="mt-4 text-slate-800 text-[9px] uppercase font-black hover:text-red-500 transition-colors text-center">Eliminar Acceso</button>
          </div>
        ))}
      </div>
      {qr && (
        ...
        {(state.modules || []).map((m: any) => (
          <div key={m.id} className="bg-[#0d111c] rounded-[3rem] border border-gray-800 overflow-hidden shadow-xl flex flex-col">
             <div className="bg-[#161e2e] p-6 border-b border-gray-800 flex justify-between items-center">
               ...
               <button onClick={() => confirm("¿Borrar?") && update({ modules: state.modules.filter((i: any) => i.id !== m.id) })} className="text-slate-800 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
             </div>
             <div className="p-8 space-y-6">
                ...
                {activeMod === m.id ? (
                  ...
                  <button onClick={() => setActiveMod(m.id)} className="w-full py-5 border-2 border-dashed border-gray-800 rounded-2xl text-slate-800 font-black uppercase text-[10px] hover:border-blue-500/50 hover:text-blue-500 transition-all">
                    <Plus size={16} className="inline mr-2" /> Agregar Archivo
                  </button>
                )}
             </div>
          </div>
  ...
  return (
    <div className="animate-in fade-in max-w-2xl mx-auto py-4 text-left">
      <div className="bg-[#0d111c] p-8 md:p-12 rounded-[3.5rem] border border-gray-800 space-y-10 shadow-2xl overflow-hidden">
        <h2 className="text-white text-2xl font-black italic uppercase">Instructor Responsable</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            <label className="text-slate-600 text-[10px] font-black uppercase px-2">Nombre Completo</label>
            <input value={state.instructor?.name || ""} onChange={e => update({ instructor: { ...state.instructor, name: e.target.value.toUpperCase() } })} placeholder="EJ: ING. GÓMEZ" className="w-full bg-[#111827] border border-gray-800 text-white p-5 rounded-2xl font-bold uppercase text-sm outline-none focus:border-blue-500 transition-all shadow-inner" />
          </div>
          <div className="space-y-2">
            <label className="text-slate-600 text-[10px] font-black uppercase px-2">Cargo</label>
            <input value={state.instructor?.role || ""} onChange={e => update({ instructor: { ...state.instructor, role: e.target.value.toUpperCase() } })} placeholder="EJ: SEGURIDAD" className="w-full bg-[#111827] border border-gray-800 text-white p-5 rounded-2xl font-bold uppercase text-sm outline-none focus:border-blue-500 shadow-inner" />
          </div>
        </div>
        <div className="space-y-4">
        ...
        <button onClick={handleSave} className={`w-full py-6 rounded-[2.5rem] font-black uppercase text-xs shadow-xl transition-all active:scale-95 ${saveSuccess ? 'bg-green-600 text-white' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
          {saveSuccess ? "Perfil Sincronizado" : "Guardar Perfil"}
        </button>
      </div>
    </div>
  );
...
const UserPortal = ({ state, onSubmit, onSync }: any) => {
  const sigRef = useRef<SignatureCanvas>(null);
  const [step, setStep] = useState(1), [name, setName] = useState(""), [dni, setDni] = useState(""), [done, setDone] = useState(false), [isSubmitting, setIsSubmitting] = useState(false), [cloudErr, setCloudErr] = useState(false);
  ...
  const generateReceipt = () => {
    ...
    doc.setTextColor(255, 255, 255); doc.setFontSize(24); doc.text("Constancia de Capacitación", 15, 28);
  ...
  const handleFinalSubmit = async () => {
    ...
    setCloudErr(false);
    try {
      const sig = sigRef.current!.toDataURL('image/png', 0.5); // Comprimir al 50% para reducir tamaño
  ...
  if (done) return (
    <div className="max-w-md mx-auto py-24 text-center animate-in zoom-in p-6">
      ...
      <h2 className="text-3xl font-black text-white italic uppercase mb-12">¡REGISTRO COMPLETADO!<br/><span className="text-blue-500 text-sm normal-case">Su firma se ha guardado exitosamente.</span></h2>
      ...
      <button onClick={() => window.location.href = window.location.origin + window.location.pathname} className="w-full bg-slate-800 text-white py-5 rounded-[2.5rem] font-black uppercase opacity-60 hover:opacity-100 transition-all text-[10px]">Salir</button>
    </div>
  );
  ...
  return (
    <div className="max-w-md mx-auto py-6 md:py-12 px-4 animate-in slide-in-from-bottom-10">
      <div className="text-center mb-10">
        <h1 className="text-[clamp(1.5rem,8vw,2.5rem)] font-black italic text-white uppercase mb-2 leading-none">{mo?.name || "Cargando..."}</h1>
          ...
          <div className="space-y-8 text-left">
            {mo && (
              <div className="space-y-5">
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest px-2 italic">Contenido de Curso</p>
                <div className="grid gap-4">
                  ...
                  {(!mo.documents || mo.documents.length === 0) && <p className="text-[10px] text-slate-700 italic text-center py-4">No hay manuales adjuntos</p>}
                </div>
              </div>
            )}
            <div className="space-y-6 pt-10 border-t border-gray-800/50">
              <p className="text-[10px] text-slate-600 font-black uppercase text-center italic">Identificación Empleado</p>
              <div className="space-y-5">
                ...
                <input value={dni} onChange={e => setDni(e.target.value)} placeholder="N° DOCUMENTO (DNI)" className="w-full bg-[#0d111c] border border-gray-800 text-white p-5 md:p-6 rounded-2xl font-bold outline-none focus:border-blue-500 shadow-inner" />
              </div>
              <button onClick={() => { if(!name || !dni) return alert("Faltan datos."); setStep(2); }} className="w-full bg-blue-600 text-white py-5 md:py-6 rounded-[2.5rem] font-black uppercase shadow-xl mt-6 active:scale-95 transition-all flex items-center justify-center gap-3">Continuar <ChevronRight size={18} /></button>
            </div>
          </div>
        ) : (
          <div className="space-y-8 animate-in fade-in">
            <div className="space-y-4">
              <div className="flex justify-between items-center px-4">
                <label className="text-[10px] font-black uppercase text-slate-500 italic">Dibuje su firma aquí</label>
            ...
            {cloudErr && (
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-center gap-3 animate-in shake">
                 ...
                 <p className="text-red-400 text-[10px] font-bold">Error de red. Verifique su conexión y reintente.</p>
              </div>
            )}
            <div className="flex gap-4">
              ...
              <button disabled={isSubmitting} onClick={handleFinalSubmit} className="flex-[2] bg-blue-600 text-white py-5 md:py-6 rounded-[2.5rem] font-black uppercase shadow-xl active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50 transition-all">
                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : "Finalizar Registro"}
              </button>
            </div>
          </div>
        )}
...
// --- Iconos ---
const KeyIcon = ({ size, className }: any) => <Key size={size} className={className} />;
